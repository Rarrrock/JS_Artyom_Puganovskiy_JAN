...
let divPersonContainer = document.createElement('div');

for (const person of simpsons) {
    let divPerson = document.createElement('div');
    divPerson.classList.add('member');

    let headerPerson = document.createElement('h1');
    headerPerson.classList.add('heading2');
    headerPerson.innerText = 'Person:';
    divPerson.appendChild(headerPerson);

    for (const key in person) { // тут ВАЖНО использовать "in" вместо "of" !!!
        let paragraphPerson = document.createElement('p')
        paragraphPerson.innerText = key + ': ' + person[key];
        divPerson.appendChild(paragraphPerson);
    }
    divPersonContainer.appendChild(divPerson);
}
